...
import { PandocInfo, getPandocInfo } from './pandoc_info';
...
export async function activate(extensionContext: vscode.ExtensionContext) {
	...
	const pandocInfo = await getPandocInfo(config);
...
function showPandocMissingError(checkModifiedSystem?: boolean) {
	...
	let message: string;
	if (extensionState.pandocInfo === undefined) {
		message = [
...
function showPandocVersionMessage(checkModifiedSystem?: boolean) {
	if (checkModifiedSystem) {
		...
		getPandocInfo(extensionState.config).then((result) => {
			...
			extensionState.pandocInfo = result;
			if ((result && result.versionString !== oldPandocInfo?.versionString) || (!result && result !== oldPandocInfo)) {
	...
	if (!extensionState.pandocInfo?.isMinVersionRecommended) {
		messageArray.push(
			`Pandoc ${extensionState.pandocInfo?.versionString} is installed, but ${extensionState.pandocInfo?.minVersionRecommendedString}+ is recommended.`,
		);
		if (!extensionState.pandocInfo?.supportsCodebraidWrappers) {
			messageArray.push(
